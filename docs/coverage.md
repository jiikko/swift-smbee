...
| timeout/progress/cancellation | operational | `SMBTransport.swift`, `SMBOperationDeadline.swift`, `SMBTransferProgress.swift`, `SMB2Cancel` | yes | yes | no | no | implemented-but-underverified | Socket-level timeout is broadly wired. `smbcli --operation-timeout` wraps all commands; credential and credential-provider overloads for read, upload, and recursive download/upload/copy/delete expose operation deadlines while legacy nonescaping provider calls remain source-compatible. Unit tests cover provider resolution and post-auth blocking receive deadlines, including single and recursive upload; Samba E2E covers successful recursive operations and deletion results. Deadline errors are `SMBTransportError.timedOut`. A deadline is cooperative client-side cancellation and does not roll back completed side effects. Windows/NAS cancellation smoke remains. |
