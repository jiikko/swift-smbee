...
| READ full/range/streaming | MS-SMB2 3.2.4.6 | `SMBClient.swift`, `SMBee.withReadStream` | yes | yes | smoke | no | covered | CLI size verification exists for single-file and recursive get. PR/push covers 4GiB-boundary range reads; weekly CI covers a sparse 4GiB+ read stream and EOF. |
...
| CreditCharge / CreditRequest | MS-SMB2 3.2.4.1 | `SMB2Credit`, `SMB2CreditWindow`, `SMB2Read`, `SMB2Write`, `SMBSession`, `SMBTransferLimits` | yes | yes | no | no | covered | READ/WRITE charge, response grant tracking, credit-capped chunk planning, credit-window blocking reserve/grant, and messageId-based multi-flight response demux are implemented. Push CI covers encrypted 2MiB+ READ/WRITE and multi-credit WRITE; weekly CI covers a sparse 4GiB+ read stream. |
