...
- `SMBee.read(...)`、単一/再帰 upload、recursive download/copy/delete は `operationTimeout` で操作全体の deadline を指定できる。`timeout` は socket I/O、`perFileTimeout` は再帰転送の各ファイルに適用され、deadline 超過は `SMBTransportError.timedOut` になる。
