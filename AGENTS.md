...
- Performance-sensitive work is complete only after recording the run URL and comparing the 10-invocation medians with the previous comparable successful master run. Report material improvements, regressions, and the observed spread; do not infer an improvement from a single raw sample.
