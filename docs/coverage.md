...
| byte-range lock | MS-SMB2 LOCK 2.2.26, 3.2.4.19 | `SMB2Lock`, `SMBClientSession.withFileLock` | yes | yes | no | no | covered | Scoped lock/unlock on a dedicated open handle. Lock sequence tracking (resilient/durable reconnect) is unsupported. CLI surface is not provided. |
| sparse file | MS-FSCC FSCTL_SET_SPARSE / SET_ZERO_DATA / QUERY_ALLOCATED_RANGES | `SMB2SparseFile`, `SMBClientSession.setSparse/zeroRange/allocatedRanges`, `smbcli sparse` | yes | yes | no | no | covered | Filesystem-dependent; servers may return STATUS_INVALID_DEVICE_REQUEST. |
