...
| CreditCharge / CreditRequest | MS-SMB2 3.2.4.1 | `SMB2Credit`, `SMB2CreditWindow`, `SMB2Read`, `SMB2Write`, `SMBSession`, `SMBTransferLimits` | yes | no | no | no | partial | READ/WRITE charge, response grant tracking, credit-capped chunk planning, and credit-window blocking reserve/grant are implemented. Multi-flight response demux is not implemented. |
