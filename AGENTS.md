...
- Performance-sensitive work is complete only after recording the run URL and comparing the current and previous-master 10-invocation medians measured on the same runner. Report material improvements, regressions, and the observed spread; do not infer an improvement from a single raw sample.
