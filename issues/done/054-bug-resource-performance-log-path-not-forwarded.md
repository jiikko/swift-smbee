...
- [x] カスタムログパスのサポートまたは明示拒否の仕様がスクリプトに記載されている。
- [x] サポートする場合、コンテナ内の書き込み先とホスト側の artifact/summary の読み取り先が同一入力から決まる（明示拒否方針のため対象外）。
- [x] 拒否する場合、デフォルト以外の指定が非ゼロ終了し、理由が標準エラーに出る。
- [x] `bin/ci/test-performance-scripts` にカスタムパスの回帰テストがあり、実際のファイル存在と読み取り結果を検証する。
