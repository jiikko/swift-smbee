...
| QUERY_INFO stat | MS-SMB2, MS-FSCC | `SMB2ReadCodecs.swift`, `SMBClient.swift` | yes | yes | smoke | no | covered | Reparse tag/kind is reported; target data is read separately through `readlink`. |
| READ full/range/streaming | MS-SMB2 3.2.4.6 | `SMBClient.swift`, `SMBee.withReadStream` | yes | yes | smoke | no | covered | CLI size verification exists for single-file and recursive get. PR/push covers 4GiB-boundary range reads including a 2MiB streaming read crossing UInt32.max. A full 4GiB+ read stream with EOF verification is not automated (Linux pure-Swift CCM throughput, issue 075); `testReadStreamCountsFileLargerThan4GiB` remains behind `SMBEE_E2E_LARGE=1`. |
...
| CreditCharge / CreditRequest | MS-SMB2 3.2.4.1 | `SMB2Credit`, `SMB2CreditWindow`, `SMB2Read`, `SMB2Write`, `SMBSession`, `SMBTransferLimits` | yes | yes | no | no | covered | READ/WRITE charge, response grant tracking, credit-capped chunk planning, credit-window blocking reserve/grant, and messageId-based multi-flight response demux are implemented. Push CI covers encrypted 2MiB+ READ/WRITE, multi-credit WRITE, and a 2MiB streaming read crossing the 4GiB boundary. Full 4GiB+ read-stream automation is deferred to issue 075. |
...
| DFS referral metadata / auto-follow | MS-DFSC, MS-FSCC FSCTL_DFS_GET_REFERRALS | `SMB2DfsReferral.swift`, `SMBClient.dfsReferral/resolveDFS/connectFollowingDFS/listFollowingDFS/readFollowingDFS` | yes | yes | no | no | covered | Multi-hop resolution, loop detection, credential-scoped bounded TTL cache, suffix rewrite, and Samba msdfs chain E2E are covered. Windows DFS remains pending. |
| reparse point metadata / target | MS-FSCC FileAttributeTagInformation, FSCTL_GET_REPARSE_POINT | `SMB2ReadCodecs.swift`, `SMBFileStat`, `SMBee.readlink`, `smbcli readlink` | yes | yes | no | no | underverified | Samba 4.22 E2E creates an actual relative symlink reparse point over SMB, verifies stat/readlink, copy skip, link-only recursive delete, and target survival. Windows/macOS SMBX smoke remains. DFS/NFS reparse data stays opaque by specification. |
...
| timeout/progress/cancellation | operational | `SMBTransport.swift`, `SMBOperationDeadline.swift`, `SMBTransferProgress.swift`, `SMB2Cancel` | yes | yes | no | no | implemented-but-underverified | Socket-level timeout is broadly wired. `smbcli --operation-timeout` wraps all commands; credential and credential-provider overloads for read, upload, and recursive download/upload/copy/delete expose operation deadlines while legacy nonescaping provider calls remain source-compatible. Unit tests cover provider resolution and post-auth blocking receive deadlines, including single and recursive upload; Samba E2E covers successful recursive operations and deletion results. Deadline errors are `SMBTransportError.timedOut`. A deadline requests cooperative client-side cancellation and waits for the operation task to finish, so it may return after the configured duration; it does not roll back completed side effects. Windows/NAS cancellation smoke remains. |
