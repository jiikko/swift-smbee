...
| Linux | Samba 4.22 container | Debian 13 profile `smb422-reparse` | 3.0.2 | required / CMAC | required / CCM | NTLMv2 password | automated PR/push reparse E2E | Creates a relative symlink reparse point over SMB; verifies stat/readlink, recursive copy skip, link-only delete, and external-target survival. |
